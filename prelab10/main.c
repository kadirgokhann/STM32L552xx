...
void ADC1_2_IRQHandler() {
	num=ADC_DR;
	SET1(ADC_CR, 2); //Write 1 to start regular conversions
}
...
void init_uart(){
    ...
    Enable_RCC_AHB2ENR(PortG);
	//Change the functionality of the pin
    SET_MODER_ALT_FUNC(GPIOG->MODER, 7);
    SET_MODER_ALT_FUNC(GPIOG->MODER, 8);
    //Change the alternate function of the pin
    // GP7: LPUART1_TX AF8
    SET0(GPIOG->AFRL, 28);
    SET0(GPIOG->AFRL, 29);
    SET0(GPIOG->AFRL, 30);
    ...
    // 106666.66666666667 = 4 000 000 Hz * 256 / 9600
    // 0x1a0aa =~ 106666.66666666667
    LPUART1->BRR = 0x1a0aa;
...
void LPUART1_IRQHandler() {
    char f=(int)(num/1000)+48;

    uint32_t diff=num;
    while(diff>=1000)
    diff=diff-1000;
    char t=(int)(diff/100)+48;

    diff=num;
    while(diff>=100)
    diff=diff-100;
    char s=(int)(diff/10)+48;
